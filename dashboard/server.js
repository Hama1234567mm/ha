...
class Dashboard {
    ...
    // parse URL-encoded forms (login form posts)
    this.app.use(express_1.default.urlencoded({ extended: true }));
        ...
        // Simple JS prompt-based login flow driven by config.auth
        // If auth is enabled in config, show a browser prompt that posts credentials to /auth/login
        this.app.use((req, res, next) => {
            try {
                const auth = config_1.default && config_1.default.dashboard && config_1.default.dashboard.auth;
                // Only enforce for enabled auth and for non-auth, non-api requests
                if (auth && auth.enabled) {
                    // Allow access to auth endpoints and API routes without prompting
                    if (req.path.startsWith('/auth') || req.path.startsWith('/api') || req.path.startsWith('/assets'))
                        return next();
                    // If already authenticated in session, proceed
                    if (req.session && req.session.dashboardAuthenticated)
                        return next();

                    // Redirect to the login page and include the originally requested path
    ...
    routes() {
        // Auth endpoints: render login page and handle login/logout
        this.app.get('/auth/login', (req, res) => {
            try {
                const auth = config_1.default && config_1.default.dashboard && config_1.default.dashboard.auth;
                if (!auth || !auth.enabled) {
                    return res.status(400).send('Auth not enabled');
                }
                const nextUrl = req.query.next || '/';
                return res.render('auth/login', { title: 'Login', next: nextUrl, config: config_1.default, layout: 'layouts/auth' });
            }
            catch (error) {
                console.error('Error rendering /auth/login:', error);
        ...
        this.app.post('/auth/login', (req, res) => {
            try {
                ...
                if (username === auth.username && password === auth.password) {
                    ...
                    // If request expects JSON, return JSON; otherwise redirect to next
                ...
                // For form submits, re-render login with an error
                return res.status(401).render('auth/login', { title: 'Login', error: 'Invalid credentials', next: req.body.next || '/', layout: 'layouts/auth' });
            }
            catch (error) {
        ...
        this.app.post('/api/commands/toggle', async (req, res) => {
            try {
                ...
                const settingsPath = (0, path_2.join)(process.cwd(), 'dist\\settings.json');
        ...
        this.app.get('/autoreply', async (_req, res) => {
            try {
                const settings = JSON.parse((0, fs_1.readFileSync)((0, path_2.join)(process.cwd(), 'dist\\settings.json'), 'utf8'));
        ...
        this.app.get('/suggestions', async (_req, res) => {
            try {
                const settings = JSON.parse((0, fs_1.readFileSync)((0, path_2.join)(process.cwd(), 'dist\\settings.json'), 'utf-8'));
    ...
    async saveSettings(settings) {
        try {
            const settingsPath = (0, path_2.join)(__dirname, 'dist\\settings.json');
            ...
            delete require.cache[require.resolve('dist\\settings.json')];
